...
var loadOrderItems = function (result) {
  idsOfRelevantOrders = me.removeLocalIDs(me.getPropertyValuesFromArray(result.AssignedTourOrders, "Id"));
  if (idsOfRelevantOrders.length > 0) {
    request.addRequest('NFT_Tour_Order_Items', idsOfRelevantOrders);  
  }
};
  ...
  promise = me.getWorkingDays().then(
    function (workingDayRange){
      sqlBulkJsonArray = [
        ...
        {
          dataSource       : "DsBoSfReplicationCallbacks",
          dataSourceMethod : "AssignedRole",
          jsonParams       :  { userId: Facade.getSfUserId() },
          uniqueReturnKey  : "AssignedRole"	
        },
        {
          dataSource       : "DsBoSfReplicationCallbacks",
      ...
      return me.executeRequestsAsync(sqlBulkJsonArray).then(
        function(result){
          ...
          let assignedRole = me.removeLocalIDs(me.getPropertyValuesFromArray(result.AssignedRole, "usrRoleId"));
          if(assignedRole.includes('TourUser')){
