...
function validateVehicleDetails(messageCollector){
    ...
    if (considerMileage === "yes") {
      if (
        Utils.isEmptyString(odometerStart) ||
        !Utils.isDefined(odometerStart) ||
        typeof Number(odometerStart) !== "number" ||
        !Number.isInteger(Number(odometerStart))
      ) {
        newError = {
          level: "error",
          objectClass: "BoTour",
    ...
    if (considerVehicle !== "no") {
      ...
      promise = BoFactory.loadListAsync(
        ...
        if (messageCollector.store.length == 0) {
          me.setVehicleDetailsReviewed("1");
        }
      });
    } else if (messageCollector.store.length == 0) {
      me.setVehicleDetailsReviewed("1");
    }
